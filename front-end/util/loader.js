...
export function Loader() {
    ...
    this.loadHTML = async (url) => {
        ...
        if (resp.status === 200) {
            window.AppLoaderCache[url] = result;
        }
        return result;
    };
    ...
    async function _load(dependency) {
        for (let i = 0; i < dependency.length; i++) {
            if (!window[dependency[i].checkVariable] && window.AppLoaderCache.indexOf(dependency[i].url) === -1) {
                const script = document.createElement('script');
                script.src = dependency[i].url;
                document.body.appendChild(script);
                window.AppLoaderCache.push(dependency[i].url);
            }
        }
