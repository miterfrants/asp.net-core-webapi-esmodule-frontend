...
import {
    Router
} from '/route/router.js';

window['AppController'] = [];
window['AppCurrentController'] = null;
...
export const Route = {
    ...
    routing: async (path, routers, context, pMatchRouter, pParentController) => {
        if (window.AppCurrentController) {
            Route.recurisvieExitController(window.AppCurrentController);
        }
    ...
    // client side first time: constructor -> init -> enter -> render -> postRender -> exit
    ...
    executeController: async (controller, context, htmlPath, parentController) => {
        ...
        const instances = window.AppController.filter((instance) => {
        ...
        if (instances.length === 0) {
            ...
            window.AppController.push(controllerInstance);
        } else {
